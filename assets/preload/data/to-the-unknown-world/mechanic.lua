...
function onCreate()
	  ...
	  makeLuaText('cantAtt', 'You can`t attack', 300, 430, 85)
	  addLuaText('cantAtt', true)
	  setTextSize('cantAtt', 30)
	  setProperty('cantAtt.visible', false)

	if getPropertyFromClass('ClientPrefs', 'downScroll') then
	   setProperty('cantAtt.y', 605)
    end	
	makeAnimatedLuaSprite('attackB', 'virtualbuttons', 0, 582.5);
	addAnimationByPrefix('attackB', 'anonPress', 'anonPress', 24, false);
	addAnimationByPrefix('attackB', 'aPressed', 'aPressed', 12, false);
	addLuaSprite('attackB', false);
...
function onUpdate()
  ...
  if canAttack == true and (getMouseX('camHUD') > 0 and getMouseX('camHUD') < 128) and (getMouseY('camHUD') > 582.5 and getMouseY('camHUD') < 720 and mouseReleased('left')) then
     characterPlayAnim('boyfriend', 'attack', 16, true)
	 runTimer('cooldownBF', 0.01, 1)
  end
  
  if canAttack == true and (getMouseX('camHUD') > 0 and getMouseX('camHUD') < 128) and (getMouseY('camHUD') > 582.5 and getMouseY('camHUD') < 720 and mouseReleased('left')) and getProperty('hippo.x') > getProperty('boyfriend.x')- 385 then
     characterPlayAnim('boyfriend', 'attack', 16, true)
  ...
  if canAttack == true and (getMouseX('camHUD') > 0 and getMouseX('camHUD') < 128) and (getMouseY('camHUD') > 582.5 and getMouseY('camHUD') < 720 and mouseReleased('left')) and getProperty('sae.x') > getProperty('boyfriend.x')- 385 then
     characterPlayAnim('boyfriend', 'attack', 16, true)
	 playSound('bfattack', 1, 1)	 
    ...
    if (getMouseX('camHUD') > 0 and getMouseX('camHUD') < 128) and (getMouseY('camHUD') > 582.5 and getMouseY('camHUD') < 720 and mouseReleased('left')) then
  ...
  if canAttack == false then
    setProperty('cantAtt.visible', true)
  end	
end
...
function onTimerCompleted(tag, loops, loopsLeft)
	 ...
	 if tag == 'Hippscore' then
	    setProperty('songScore', getProperty('songScore')+ getRandomInt(7000, 9550))
     end	   
       --Sael Alien Timers--	 
     if tag == 'Saewalk' then
	    Swalk = true
	 ...
	 if tag == 'Saescore' then
	    setProperty('songScore', getProperty('songScore')+ getRandomInt(11500, 15150))	  
     end	   
	 
	 if tag == 'cooldownBF' then
	    canAttack = false
		setProperty('cantAtt.color', getColorFromHex('878787'))
		doTweenColor('canAtt', 'cantAtt', 'FFFFFF', 3, 'bounceOut')
		runTimer('cooldownBFcomplete', 3, 1)
	 end
	 if tag == 'cooldownBFcomplete' then
	    canAttack = true
	    setProperty('cantAtt.visible', false)		
	 end
end
